...
\begin{enumerate}
\color{darkblue}
\item $M$, die dieZahlen 1, 2, 3, 4, 5, 6 und 7 beinhaltet.
\item $G$. die alle Zahlen der Menge M beinhaltet, die größer als 3 sind.  
\end{enumerate}
...
\begin{enumerate}
\color{darkblue}
\item $M$ mit den Zahlen 1, 2, 3, 4, 5, 6 und 7.  

\textcolor{black}{$M := \{1,2,3,4,5,6,7\}$}
\item $G$ mit allen Zahlen der Menge M, die größer als 3 sind.  
...
\color{darkblue}
Schreiben Sie, falls möglich Mengen als Teilmengen anderer Mengen auf und notieren Sie Mengengleichheiten, fals bestehend.
...
\color{darkblue}
Schreiben Sie, falls möglich Mengen als Teilmengen anderer Mengen auf und notieren Sie Mengengleichheiten, falls bestehend.
...
\item Es seien $A := \{5,6,7,8\}$, $B := \{x\in A|x>6\}$, $C := \{5\}$.  

\textcolor{black}{$\Leftrightarrow A := \{5,6,7,8\}$, $B := \{7,8\}$, $C := \{5\}$}
...
* Für $N := \{5,6\}$ gilt $\mathcal{P}(N) = \{\emptyset, \{5\},\{6\},\{5,6\} \}$.
...
\color{darkblue}
Geben Sie alle Partitionen von $M := \{1,2,3,4\}$ an.
